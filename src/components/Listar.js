import React from  'react';
import { useEffect } from 'react';
import { useState } from 'react';
import { TouchableHighlight, Text, View, StyleSheet, ActivityIndicator } from "react-native";
import {ListItem, Avatar} from "react-native-elements"
import firebase from '../config/firebase';

export function ListarAlunos(props){

    const [alunos, setAlunos] = useState([]);

    useEffect(() => {
        firebase.db.collection("alunos").onSnapshot(querySnapshot => {
            const alunos = []
            querySnapshot.docs.forEach(doc => {
                const {nome, curso} = doc.data()
                alunos.push({
                    id: doc.id,
                    nome,
                    curso
                })
            })
            setAlunos(alunos)
        })
    }, [])
    return(
        <View style={styles.container}>
            {
                alunos.map(aluno => {
                    return(
                        <ListItem key={aluno.id} onPress={() => props.navigation.navigate("DetalhesAlunos", {alunoId: aluno.id})}>
                            <Avatar source={{uri: 'data:image/png;base64,'}} rounded/>
                            <ListItem.Content style={styles.infosAlunos}>
                                <ListItem.Title style={styles.title}>
                                    {aluno.nome}
                                </ListItem.Title>
                                <ListItem.Subtitle style={styles.subtitle}>
                                    {aluno.curso}
                                </ListItem.Subtitle>
                            </ListItem.Content>
                        </ListItem>                
                    )
                })
            }
            <TouchableHighlight style={styles.buttonStyle} onPress={() => props.navigation.navigate("AdicionarAlunos")}>
                <Text style={styles.textButton}>Cadastrar novo aluno</Text>
            </TouchableHighlight>
        </View>
    )
}

const styles = StyleSheet.create({
    container:{
        flex: 1,
        padding: 10,
        backgroundColor: '#623D74',
    },
    infosAlunos:{
        backgroundColor: '#A7A7A7',
        width: 300,
        marginHorizontal: 'auto',
        marginBottom: 10,
        padding: 20,
        borderRadius: 3,
        borderRadius: 10,
        shadowColor: 'black',
        shadowOffset:{width: 0, height: 2},
        shadowRadius: 2,
        elevation: 5
    },
    title:{
        fontSize: 20,
        fontWeight: 'bold',
        color: '#4E2064'
    },
    subtitle:{
        color: '#FFFFFF'
    },
    buttonStyle:{
        backgroundColor: '#4E2064',
        marginHorizontal: 'auto',
        marginTop: 20,
        width: 200,
        padding: 15,
        borderRadius: 3,
        borderRadius: 10,
        shadowColor: 'black',
        shadowOffset:{width: 0, height: 2},
        shadowRadius: 2,
        elevation: 5
    },
    textButton:{
        color: '#ccc',
        textAlign: 'center',
        fontSize: 18
    },
    buttonsDetails:{
        width: 80,
        textAlign: 'center',
        marginHorizontal: 180,
    },
    update: {
        backgroundColor: '#20644B',  
        padding: 10,
        marginBottom: 8
    },
    delete: {
        backgroundColor: '#8F0101',
        padding: 10
    },
    textDetail:{
        fontWeight: 'bold',
        color: '#FFFFFF'
    }
})
export default ListarAlunos;